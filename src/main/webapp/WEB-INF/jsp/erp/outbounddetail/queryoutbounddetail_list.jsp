...
<body class="no-skin">
								...
								<table class="table table-striped table-bordered table-hover"
									...
									<c:choose>
										<c:when test="${not empty pdList}">
											<c:forEach items="${pdList}" var="pd" varStatus="pds">
												<tr>
													<th class="center">出库单号:</th>
													<th class="center">${pd.out_bound_code}</th>
													<th class="center">出库通知单号：</th>
													<th class="center">${pd.outbound_notice_code}</th>
													<th class="center">出库类型:</th>
													<th class="center">${pd.type}</th>
												...
												<tr>
													...
													<th class="center">出库人:</th>
													<th class="center">${pd.createby}</th>
													<th class="center">出库时间:</th>
													<th class="center">
														<fmt:formatDate value="${pd.create_time}" pattern="yyyy-MM-dd"/>
													</th>
												</tr>
											</c:forEach>
										</c:when>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<%-- <td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.outbound_detail_id}" class="ace" /><span class="lbl"></span></label>
											</td> --%>
											...
											<%-- <td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.outbound_detail_id}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.outbound_detail_id}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.outbound_detail_id}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.outbound_detail_id}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>outbounddetail/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>outbounddetail/delete.do?outbound_detail_id="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>outbounddetail/goEdit.do?outbound_detail_id='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>outbounddetail/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>outbounddetail/excel.do';
		}
