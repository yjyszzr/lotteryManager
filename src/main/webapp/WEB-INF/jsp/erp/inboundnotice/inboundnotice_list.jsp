...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="inboundnotice/list.do" method="post" name="Form" id="Form">
						<table class="pgt-query-form-table">
							<tr>
								<td class="pgt-query-form-td">
									入库通知单：<input type="text" placeholder="入库通知单" class="nav-search-input" id="nav-search-input" autocomplete="off" name="sh_direct_sales_code" id="sh_direct_sales_code" value="${pd.sh_direct_sales_code}" placeholder="入库通知单" style="width: 120px;"/>
								</td>
								
								<td class="pgt-query-form-td">
									业务单据：<input type="text" placeholder="业务单据" class="nav-search-input"  autocomplete="off" name="sh_purchase_code" value="${pd.sh_purchase_code}" placeholder="业务单据编码"  style="width: 120px;"/>
								</td>
								...
								<td class="pgt-query-form-td">
								 	入库类型：<select name="sh_type" id="sh_type" data-placeholder="单据类型" style="width: 120px;">
								...
								<td class="pgt-query-form-td">
									...
									<c:if test="${pd.sh_status != '0'&&pd.sh_status != '1'&&pd.sh_status != '9'}">
								  	...
								  	<td class="pgt-query-form-td">
									 	入库仓库：<select  name="stores" id="stores" data-placeholder="仓库列表" style="width: 150px;">
											...
											<td class="center">
												...
												<c:if test="${QX.edit == 1 }">
														<c:if test="${var.status=='0'}">
															<a class="btn btn-xs btn-success" title="到货" onclick="arrived('${var.inbound_notice_id}','${var.inbound_notice_code}');">到货</a>
															<%-- <a class="btn btn-xs btn-success" title="质检" disabled = "ture" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','1');">质检</a> --%>
															<c:if test="${var.bill_type=='1' || var.bill_type=='2'}">
																<a class="btn btn-xs btn-success" title="批次" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','4','${var.store_sn}','${var.bill_type}','${var.status}');">批次</a>
															</c:if>
															<c:if test="${var.bill_type!='1' && var.bill_type!='2'}">
																<a class="btn btn-xs btn-success" title="批次" disabled = "ture">批次</a>
															</c:if>
															<a class="btn btn-xs btn-success" title="入库" disabled = "ture" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','2','${var.store_sn}','${var.bill_type}');">入库</a>
															<a class="btn btn-xs btn-success" title="完成" onclick="update('${var.inbound_notice_id}','${var.inbound_notice_code}','${var.bill_type}');">完成</a>
															<a class="btn btn-xs btn-success" title="明细" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','3');">明细</a>
															<a class="btn btn-xs btn-success" title="打印" disabled = "ture" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','5');">打印</a>
														</c:if>
														<c:if test="${var.status=='1'}">
															<a class="btn btn-xs btn-success" title="到货" disabled = "ture" onclick="arrived('${var.inbound_notice_id}','${var.inbound_notice_code}');">到货</a>
															<%-- <c:if test="${var.bill_type != '1' && var.bill_type != '2'}">
																<a class="btn btn-xs btn-success" title="质检" disabled = "ture" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','1');">质检</a>
															</c:if> --%>
															<%-- <c:if test="${var.bill_type == '1' || var.bill_type == '2'}">
															...
															<a class="btn btn-xs btn-success" title="入库" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','2','${var.store_sn}','${var.bill_type}');">入库</a>
															<a class="btn btn-xs btn-success" title="完成" onclick="update('${var.inbound_notice_id}','${var.inbound_notice_code}','${var.bill_type}');">完成</a>
															<a class="btn btn-xs btn-success" title="明细" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','3');">明细</a>
														...
														<c:if test="${var.status=='9'}">
															...
															<a class="btn btn-xs btn-success" title="入库" disabled = "ture" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','2','${var.store_sn}','${var.bill_type}');">入库</a>
															<a class="btn btn-xs btn-success" title="完成" disabled = "ture" onclick="update('${var.inbound_notice_id}','${var.inbound_notice_code}','${var.bill_type}');">完成</a>
															<a class="btn btn-xs btn-success" title="明细" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','3');">明细</a>
															<a class="btn btn-xs btn-success" title="打印" onclick="quality('${var.inbound_notice_code}','${var.bill_type}','5');">打印</a>
														</c:if>
												</c:if>
	...
	<script type="text/javascript">
		...
		//更新到货通知单状态
		function update(Id,inbound_code,bill_type){
			var str = "此通知单已完成!";
			bootbox.confirm(str, function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>inboundnotice/update.do?inbound_notice_id="+Id+"&inbound_code="+inbound_code+"&bill_type="+bill_type+"&tm="+new Date().getTime();
					$.get(url,function(data){
						top.hangge();
						if(data.msg=="fail"){
							alert(data.info);
		...
		function quality(code,type,page,store_sn,bill_type,status){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 if(type == '1' || type == '2'){
			 ...
			 diag.URL = '<%=basePath%>inboundnoticedetail/list.do?inbound_notice_code='+code+'&bill_type='+type+'&page='+page+'&store_sn='+store_sn+'&bill_type='+bill_type+'&noticeStatus='+status;
			 diag.Width = 1250;
			 diag.Height = 755;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
