						...
						<table style="margin-top:5px;">
							<tr>
							    <td style="vertical-align:top;padding-left:2px;">
								 	<select class="chosen-select form-control" name="inbound_type" id="id" data-placeholder="请选择" style="vertical-align:top;width: 160px;">
									<option value=""></option>
									<option value="">全部</option>
									<option value="1" ${pd.inbound_type=="1"?'selected':''} >采购入库</option>
									<option value="2" ${pd.inbound_type=="2"?'selected':''} >生产入库</option>
									<option value="3" ${pd.inbound_type=="3"?'selected':''} >调拨入库</option>
									<option value="4" ${pd.inbound_type=="4"?'selected':''}>客户退货入库</option>
									<option value="5" ${pd.inbound_type=="5"?'selected':''}>领料退货入库</option>
								  	</select>
								...
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="业务单号输入" class="nav-search-input" id="nav-search-input" autocomplete="off" name="purchase_code" value="${pd.purchase_code}" placeholder="这里输入关键词"/>
								...
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="到货通知单号输入" class="nav-search-input" id="nav-search-input" autocomplete="off" name="inbound_notice_code" value="${pd.inbound_notice_code}" placeholder="这里输入关键词"/>
											<i class="ace-icon fa fa-search nav-search-icon"></i>
										</span>
									</div>
								</td>
										...
										<tr>
											...
											<td class="center">
												...
												<c:if test="${QX.edit == 1 }">
													...
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-success" onclick="del('${var.purchase_id}');">
														<i class="ace-icon fa bigger-120" title="删除">删除</i>
													</a>
													</c:if>
														<a class="btn btn-xs btn-success"  ${var.status==PurchaseConstants.BUSINESS_STATUS_NO_COMMIT?"":"disabled='disabled'"} onclick="updateStaus('${var.purchase_id}',${PurchaseConstants.BUSINESS_STATUS_CREATE})">
														<i class="ace-icon fa bigger-120" title="提交审核">提交</i>
												</a>
