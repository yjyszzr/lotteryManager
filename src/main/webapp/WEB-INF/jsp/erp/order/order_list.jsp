							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.pgt_order_id}" class="ace" /><span class="lbl"></span></label>
											</td>
											...
											<td class='center'>${var.pickup_id}</td>
											<td class='center'>${var.shipping_status}</td>
											<td class='center'>${var.pay_status}</td>
											<td class='center'>${var.consignee}</td>
											<td class='center'>${var.region_code}</td>
											<td class='center'>${var.region_name}</td>
											<td class='center'>${var.address}</td>
											<td class='center'>${var.address_lng}</td>
											<td class='center'>${var.address_lat}</td>
											<td class='center'>${var.receiving_mode}</td>
											<td class='center'>${var.tel}</td>
											<td class='center'>${var.email}</td>
											<td class='center'>${var.postscript}</td>
											<td class='center'>${var.best_time}</td>
											<td class='center'>${var.pay_id}</td>
											<td class='center'>${var.pay_code}</td>
											<td class='center'>${var.pay_name}</td>
											<td class='center'>${var.pay_sn}</td>
											<td class='center'>${var.is_cod}</td>
											<td class='center'>${var.order_amount}</td>
											<td class='center'>${var.order_points}</td>
											<td class='center'>${var.money_paid}</td>
											<td class='center'>${var.goods_amount}</td>
											<td class='center'>${var.inv_fee}</td>
											<td class='center'>${var.shipping_fee}</td>
											<td class='center'>${var.cash_more}</td>
											<td class='center'>${var.discount_fee}</td>
											<td class='center'>${var.change_amount}</td>
											<td class='center'>${var.shipping_change}</td>
											<td class='center'>${var.surplus}</td>
											<td class='center'>${var.user_surplus}</td>
											<td class='center'>${var.user_surplus_limit}</td>
											<td class='center'>${var.bonus_id}</td>
											<td class='center'>${var.shop_bonus_id}</td>
											<td class='center'>${var.bonus}</td>
											<td class='center'>${var.shop_bonus}</td>
											<td class='center'>${var.store_card_id}</td>
											<td class='center'>${var.store_card_price}</td>
											<td class='center'>${var.integral}</td>
											<td class='center'>${var.integral_money}</td>
											<td class='center'>${var.give_integral}</td>
											<td class='center'>${var.order_from}</td>
											<td class='center'>${var.add_time}</td>
											<td class='center'>${var.pay_time}</td>
											<td class='center'>${var.shipping_time}</td>
											<td class='center'>${var.confirm_time}</td>
											<td class='center'>${var.delay_days}</td>
											<td class='center'>${var.order_type}</td>
											<td class='center'>${var.service_mark}</td>
											<td class='center'>${var.send_mark}</td>
											<td class='center'>${var.shipping_mark}</td>
											<td class='center'>${var.buyer_type}</td>
											<td class='center'>${var.evaluate_status}</td>
											<td class='center'>${var.evaluate_time}</td>
											<td class='center'>${var.end_time}</td>
											<td class='center'>${var.is_distrib}</td>
											<td class='center'>${var.distrib_status}</td>
											<td class='center'>${var.is_show}</td>
											<td class='center'>${var.is_delete}</td>
											<td class='center'>${var.order_data}</td>
											<td class='center'>${var.mall_remark}</td>
											<td class='center'>${var.shop_remark}</td>
											<td class='center'>${var.store_remark}</td>
											<td class='center'>${var.close_reason}</td>
											<td class='center'>${var.cash_user_id}</td>
											<td class='center'>${var.last_time}</td>
											<td class='center'>${var.order_cancel}</td>
											<td class='center'>${var.refuse_reason}</td>
											<td class='center'>${var.sub_order_id}</td>
											<td class='center'>${var.is_freebuy}</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.pgt_order_id}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.pgt_order_id}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.pgt_order_id}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.pgt_order_id}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>order/delete.do?pgt_order_id="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>order/goEdit.do?pgt_order_id='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
