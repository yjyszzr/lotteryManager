...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="inboundnoticestockbatch/list.do" method="post" name="Form" id="Form">
						...
						<input type="hidden" name="sku_barcode" id="sku_barcode" value="${pd.sku_barcode}"/>
						<input type="hidden" name="pre_arrival_quantity" id="pre_arrival_quantity" value="${pd.pre_arrival_quantity}"/>
						<input type="hidden" name="varJsonArray" id="varJsonArray" value='${varJsonArray}'/>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<c:if test="${pd.page == 5 || pd.status != 0}">
												<td class='center'>
														<a class="btn btn-xs btn-success" title="打印" onclick="printcode('${var.inbound_notice_stock_batch_id}','${pd.inbound_notice_code}',
											...
											<c:if test="${pd.status == 0 && (pd.bill_type=='1' || pd.bill_type=='2') && pd.page != 5}">
											<td class="center">
													<c:if test="${pd.status == 0}">
														<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.inbound_notice_stock_batch_id}','${pd.inbound_notice_code}',
																'${pd.notice_detail_id}','${pd.sku_id}','${pd.sku_name}',
																'${pd.sku_encode}','${pd.sku_barcode}','${pd.unit_name}',
																'${pd.spec}','${pd.quantity}','${pd.bill_type}','${pd.status}');">编辑</a>				
														<a class="btn btn-xs btn-danger" onclick="del('${var.inbound_notice_stock_batch_id}');">删除</a>
													</c:if>
											</td>
											</c:if>
										</tr>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
								    <c:if test="${pd.status == 0 && pd.noticeStatus == 1}">
										<c:if test="${QX.add == 1 }">
										...
										<a class="btn btn-mini btn-success" onclick="submit();">提交</a>
									</c:if>
									<%-- <c:if test="${QX.del == 1 }">
	...
	<script type="text/javascript">
		...
		function submit(){
			//预计发货数量
			var preArrivalQuantity = $("#pre_arrival_quantity").val();
			//获取table有用数据
			var myArray = $("#varJsonArray").val();
			var jsonobj=eval(myArray); 
			var myobj=eval(jsonobj);  
			if(myobj.length == 0){
				bootbox.alert({  
		            buttons: {  
		               ok: {  
			...
			for(var i=0;i<myobj.length;i++){  
				total += myobj[i].quantity;  
			}
			bootbox.confirm("预计发货数量为："+preArrivalQuantity+"，提交批次数量为："+total+"，提交后将不能修改，确认是否提交？", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>inboundnoticestockbatch/submit.do?varList="+encodeURI(myArray);
					$.get(url,function(data){
						top.hangge();
						if(data.msg=="fail"){
							alert(data.info);
