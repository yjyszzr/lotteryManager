...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class="center">
												<a class="btn btn-xs btn-success" onclick="opendetail('${var.io_type}','${var.code}');">明细</a>
												<a class="btn btn-xs btn-success" ="编辑" onclick="openbatch('${var.io_type}','${var.code}');">批次</a>
											</td>
										</tr>							
									</c:forEach>
									</c:if>
	...
	<script type="text/javascript">
		$(top.hangge());//关闭加载状态
		//检索
		function opendetail(io_type,code){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title =" 详情";
		     diag.URL = '<%=basePath%>queryallocation/detail.do?io_type='+io_type+'&code='+code;
		...
		function openbatch(io_type,code){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title =" 详情";
		     diag.URL = '<%=basePath%>queryallocation/batch.do?io_type='+io_type+'&code='+code;
			 diag.Width = $(window).width();
			 diag.Height = $(window).height();
			 diag.Modal = true;				//有无遮罩窗口
			 diag.ShowMaxButton = true;	//最大化按钮
