			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="defectivegoods/checkList.do" method="post" name="Form" id="Form">
										...
										<tr>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
														<c:if test="${var.status==DefetiveConstants.BUSINESS_STATUS_SUBMIT}">
															<a class="btn btn-xs btn-success" onclick="checked('${var.defective_id}');">通过</a>
															<a class="btn btn-xs btn-success" onclick="notchecked('${var.defective_id}');">驳回</a>
														</c:if>
														<c:if test="${var.status!=DefetiveConstants.BUSINESS_STATUS_SUBMIT}">
															<a class="btn btn-xs btn-success" disabled="ture" onclick="checked('${var.defective_id}');">通过</a>
															<a class="btn btn-xs btn-success" disabled="ture" onclick="notchecked('${var.defective_id}');">驳回</a>
														</c:if>
														<a class="btn btn-xs btn-success" onclick="details('${var.defective_code}','${var.remark}');">详情</a>
													</c:if>
												</div>
											</td>
										</tr>
	...
	<script type="text/javascript">
		...
		function notchecked(Id){
			bootbox.confirm("确定要驳回吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>defectivegoods/notchecked.do?defective_id="+Id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						tosearch();
					});
				}
		...
		function checked(Id){
			bootbox.confirm("确定要通过吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>defectivegoods/checked.do?defective_id="+Id+"&tm="+new Date().getTime();
					$.get(url,function(data){
						top.hangge();
						if(data.msg=="fail"){
							alert(data.info);
		...
		function details(sn,remark){
			 ...
			 diag.Drag=true;
			 diag.Title ="明细";
			 diag.URL = '<%=basePath%>defectivegoodsdetail/listByCode.do?defective_code='+ sn +'&remark='+remark;
			 diag.Width = 1250;
			 diag.Height = 755;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
