					...
					<form action="createCode/proCode.do" name="Form" id="Form" method="post">
						...
						<table style="margin-top: 5px;">
							<tr>
								...
								<td><input type="text" name="objectName" id="objectName" value="${pd.OBJECTNAME }" placeholder="这里输入处理类名称" style="width:259px" title="类名称"/></td>
							...
							<tr>
							    <td style="width:76px;text-align: right;">主键：</td>
								<td colspan="1"><input type="text" name="primaryKey" id="primaryKey" value="${pd.PRIMARYKEY }" placeholder="主键  (不要输入特殊字符,请用纯字母)" style="width:259px" title="主键名称"/></td>
								<td>&nbsp;&nbsp;例如:<font color="red" style="font-weight: bold;">goods</font>_id&nbsp;&nbsp;只输入红色部分</td>
								<td style="width:76px;text-align: right;">主键类型：</td>
								<td><input type="text" name="primaryType" id="primaryType" value="${pd.PRIMARYTYPE }" placeholder="这里输入主键类型" style="width:156px" title="类型"/></td>
							</tr>
							<tr>
							    <td style="width:76px;text-align: right;">表名：</td>
								<td><input type="text" name="tableName" id="tableName" value="${fn:split(pd.TABLENAME, ',fh,')[0]}${fn:split(pd.TABLENAME, ',fh,')[2]}" placeholder="这里输入表名" style="width:259px" title="表名"/></td>
							</tr>
						</table>
