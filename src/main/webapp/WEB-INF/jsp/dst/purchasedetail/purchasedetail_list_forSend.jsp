...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="purchasedetail/listForSend.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'><input type="text" name="total_send_quantity" value='0' style="width:40px"></td>
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<c:if test="${QX.add == 1 }">
									<a class="btn btn-mini btn-success" onclick="updateTotalSendQuantity();">调整数量</a>
									</c:if>
	...
	<script type="text/javascript">
		...
		创建到货单
		*/
		function updateTotalSendQuantity(){
			...
			var detailStr = "";
			//发货数量详情
			for(var i=0;i < total_send_quantitys.length;i++){
				var total_send_quantity = total_send_quantitys[i];
				if(total_send_quantity.value>0){
					var inboundDetail = "{total_send_quantity:"+total_send_quantity.value+",purchase_detail_id:'"+purchase_detail_ids[i].value+"'},"
					detailStr += inboundDetail;
				}
			}
			if(detailStr.length>0){
				detailStr=detailStr.substring(0,detailStr.length-1);//去掉最后一个逗号
				jsonStr+="{quantities:["+detailStr+"]";
				jsonStr+="}";
			//	alert(jsonStr);
				//发送
				$.ajax({
					type: "POST",
					url: '<%=basePath%>purchasedetail/updateSendQuantity.do?tm='+new Date().getTime(),
			    	data: {quantityJson:jsonStr},
					dataType:'json',
					//beforeSend: validateData,
					cache: false,
					success: function(data){
