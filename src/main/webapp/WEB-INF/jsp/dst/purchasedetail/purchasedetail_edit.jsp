				...
				<div class="row">
					...
					<form action="purchasedetail/${msg }.do" name="Form" id="Form" method="post">
						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							<tr>
								<td style="width:100px;text-align: right;padding-top: 13px;">采购单编码:</td>
								<td><input type="text" name="purchase_code" readonly="readonly" id="purchase_code" value="${pd.purchase_code}" maxlength="50" placeholder="这里输入采购单编码" title="采购单编码" style="width:98%;"/></td>
							</tr>
							<%-- <tr>
							...
							<tr>
								...
								<td><input type="text" name="unit" id="unit" readonly="readonly"  value="${pd.unit}" maxlength="50" placeholder="这里输入单位" title="单位" style="width:98%;"/></td>
							</tr>
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">规格:</td>
								<td><input type="text" name="spec" id="spec" readonly="readonly" value="${pd.spec}" maxlength="50" placeholder="规格" title="规格" style="width:98%;"/></td>
							</tr>
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">数量:</td>
								<td><input type="number" name="quantity" id="quantity" min="1" value="${pd.quantity}" maxlength="32" placeholder="这里输入数量" title="数量" style="width:98%;"/></td>
							</tr>
<%-- 							<tr style='display:${msg.equals("save")?"none":"" }'>
 ...
 --%>							<tr style='display:none'>
							...
							<tr>
								...
								<td><input type="text" onblur="calcPrice()" name="tax_price" id="tax_price" value="${pd.tax_price}" maxlength="20" placeholder="这里输入含税单价,最多保留两位小数" title="含税单价" style="width:98%;"/></td>
							</tr>
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">税率:</td>
								<td><input type="number" name="tax_rate" readonly="readonly" id="tax_rate" value="${pd.tax_rate}" maxlength="5" placeholder="这里输入税率,如0.17" title="税率" style="width:98%;"/></td>
							</tr>
							<tr>
								<td style="width:75px;text-align: right;padding-top: 13px;">单价:</td>
								<td><input type="number" name="price" readonly="readonly" id="price" value="${pd.price}" maxlength="20" placeholder="这里输入单价" title="单价" style="width:98%;"/></td>
							</tr>
							<tr style='display:none'>
							...
							<tr>
								...
								<td><input class="span10" name="pre_arrival_time" id="pre_arrival_time" value="${pd.pre_arrival_time!=null?DateUtil.toDateStr(pd.pre_arrival_time):''}" type="text" data-date-format="yyyy-mm-dd" readonly="readonly" placeholder="预计到货时间" title="预计到货时间" style="width:98%;"/></td>
							</tr>
							<tr style='display:none'>
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
		...
		function save(){
			...
			if($("#tax_price").val()==""){
				...
				$("#tax_price").focus();
			return false;
			}else if( parseFloat($("#tax_price").val())<=0.01){
				$("#tax_price").tips({
					side:3,
		            msg:'请输入正确的单价',
		            bg:'#AE81FF',
		...
		$(function() {
			...
			$('#pre_arrival_time').datepicker({autoclose: true,todayHighlight: true,
				todayHighlight : true,  
		...
		function selectPurchaseDetailSku(){
			 ...
			 selectPurchaseDetailSkuDiag.URL = '<%=basePath%>goodssku/listSupplier.do?supplier_sn='+$("#supplier_sn").val()+'&rd='+Math.random();
		...
		function calcPrice(){
			...
			var price = parseFloat(tax_price)/(parseFloat(tax_rate)+1);
			//四舍五入
