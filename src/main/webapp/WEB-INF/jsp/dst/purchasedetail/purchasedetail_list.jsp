			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="purchasedetail/list.do" method="post" name="Form" id="Form">
						<input type="hidden" name="purchase_code"  value="${pd.purchase_code}" />
						<input type="hidden" name="status"  value="${pd.status}" />
						<input type="hidden" name="supplier_sn" id="supplier_sn" value="${pd.supplier_sn}" />
						<input type="hidden" name="bsType" id="bsType" value="${pd.bsType}" />
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<c:if test="${(pd.bsType==null || pd.bsType=='' ) && pd.status==PurchaseConstants.BUSINESS_STATUS_NO_COMMIT}">
 ...
 --%>											<td class='center'>${DateUtil.toDateStr(var.pre_arrival_time)}</td>
										<%-- 	<td class='center'>${var.createby}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.detail_id}');">
														编辑
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" title="删除" onclick="del('${var.detail_id}');">
														删除
													</a>
													</c:if>
												</div>
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.detail_id}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.detail_id}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									...
									<c:if test="${QX.del == 1 }">
									<a class="btn btn-mini btn-danger" onclick="makeAll('确定要删除选中的数据吗?');" title="批量删除" >删除</a>
									</c:if>
								</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>purchasedetail/goAdd.do?purchase_code=${pd.purchase_code}'+'&supplier_sn='+$("#supplier_sn").val();
			 diag.Width = 450;
			 diag.Height = 550;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑1";
			 diag.URL = '<%=basePath%>purchasedetail/goEdit.do?detail_id='+Id+'&supplier_sn='+$("#supplier_sn").val();
			 diag.Width = 450;
			 diag.Height = 550;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
