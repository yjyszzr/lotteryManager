...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="productionorder/list.do" method="post" name="Form" id="Form">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${DateUtil.toSdfTimeStr(var.purchase_date)}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.purchase_id}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.purchase_id}');">
														<i class="ace-icon fa fa-trash-o bigger-120" title="删除"></i>
													</a>
													</c:if>
													<%--待审状态显示批准核驳回按钮，如果是批准状态显示完成按钮 --%>
													<c:if test="${var.status==PurchaseConstants.BUSINESS_STATUS_CREATE}">
														<a class="btn btn-xs btn-success" onclick="updateStaus('${var.purchase_id}',${ PurchaseConstants.BUSINESS_STATUS_AGREE})">
														<i class="ace-icon fa bigger-120" title="批准">批准</i>
														<a class="btn btn-xs btn-success" onclick="updateStaus('${var.purchase_id}',${ PurchaseConstants.BUSINESS_STATUS_REJECT})">
														<i class="ace-icon fa bigger-120" title="驳回">驳回</i>
													</a>
													</c:if>
														<c:if test="${var.status==PurchaseConstants.BUSINESS_STATUS_AGREE}">
														<a class="btn btn-xs btn-success" onclick="updateStaus('${var.purchase_id}',${ PurchaseConstants.BUSINESS_STATUS_COMPLETE})">
														<i class="ace-icon fa bigger-120" title="完成">完成</i>
													</a>
													</c:if>
													<a class="btn btn-xs btn-success" onclick="toDetailList('${var.purchase_code}');">
														<i class="ace-icon fa bigger-120" title="详情">详情</i>
													</a>
													<c:if test="${var.status==PurchaseConstants.BUSINESS_STATUS_AGREE}">
													<a class="btn btn-xs btn-success" onclick="toCreateInboundNotice('${var.purchase_code}');">
														<i class="ace-icon fa bigger-120" title="生成到货">到货单</i>
													</a>
													</c:if>
													<c:if test="${var.status==PurchaseConstants.BUSINESS_STATUS_AGREE}">
													<a class="btn btn-xs btn-success" onclick="toUpdateSendQuantity('${var.purchase_code}');">
														<i class="ace-icon fa bigger-120" title="更新可发货">可发货</i>
													</a>
													</c:if>
	...
	<script type="text/javascript">
		...
		function updateStaus(Id,status){
			var promptMess = "确定要审核通过该采购单吗？";
			if(status =="${PurchaseConstants.BUSINESS_STATUS_REJECT}"){
				promptMess = "确定要驳回该采购单吗？";
			}else if(status =="${PurchaseConstants.BUSINESS_STATUS_COMPLETE}"){
				promptMess = "确定要标记完成该采购单吗？";
			}
			
			bootbox.confirm(promptMess, function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>purchase/edit.do?purchase_id="+Id+"&tm="+new Date().getTime()+"&status="+status;
					$.get(url,function(data){
						tosearch();
					});
				}
		...
		//修改
		function toDetailList(purchase_code){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="采购单详情";
		    diag.URL = '<%=basePath%>purchasedetail/list.do?purchase_code='+purchase_code;
		...
		//修改
		function toCreateInboundNotice(purchase_code){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="创建到货单";
		    diag.URL = '<%=basePath%>purchasedetail/listForInbound.do?purchase_code='+purchase_code;
 			 diag.Width = $(window).width();
			 diag.Height = $(window).height();
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
