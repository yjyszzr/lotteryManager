...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="erpwarehouse/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
								 	<select class="chosen-select form-control" name="type" id="type" data-placeholder="仓库类型" style="vertical-align:top;width: 120px;">
									<option value=""></option>
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<th class="center">上级仓库编码</th>
									<th class="center">上级仓库名称</th>
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.parent_code}</td>
											<td class='center'>${var.parent_name}</td>
											<td class='center'>
											    <c:if test="${var.warehouse_type == 0}">总仓</c:if>
											    <c:if test="${var.warehouse_type == 1}">大区仓库</c:if>
											    <c:if test="${var.warehouse_type == 2}">城市仓库</c:if>
											    <c:if test="${var.warehouse_type == 3}">门店仓库</c:if>
											</td>
											<td class='center'>${var.address}</td>
											<td class='center'>
											    <c:if test="${var.status == 0}">正常</c:if>
											    <c:if test="${var.status == 1}">冻结</c:if>
											</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>erpwarehouse/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>erpwarehouse/delete.do?warehouse_id="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>erpwarehouse/goEdit.do?warehouse_id='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>erpwarehouse/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>erpwarehouse/excel.do';
		}
	</script>
