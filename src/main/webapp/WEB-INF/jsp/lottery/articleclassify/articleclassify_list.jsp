			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="articleclassify/list.do" method="post" name="Form" id="Form">
...
<%-- 											<td class='center'>${var.id}</td> --%>
											...
											<td class='center'>
												<c:choose>
													<c:when test="${var.deleted==0}">未删除</c:when>
													<c:when test="${var.deleted==1}">已删除</c:when>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:choose>
														<c:when test="${var.deleted==0}">
															<c:if test="${QX.del == 1 }">
																<a class="btn btn-xs btn-danger" onclick="del('${var.id}','1');"   style="border-radius: 5px;" >删除</a>
															</c:if>
														</c:when>																																											
...
<%-- 									</c:if> --%>
	...
	<script type="text/javascript">
		...
		function del(Id,status){
			...
			if(status==0 ){str+="确定要上架吗?"}else{str+="确定要删除吗?"}
			bootbox.confirm(str, function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>articleclassify/delete.do?id="+Id+"&deleted="+status;
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>articleclassify/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>articleclassify/excel.do';
		}
	</script>
