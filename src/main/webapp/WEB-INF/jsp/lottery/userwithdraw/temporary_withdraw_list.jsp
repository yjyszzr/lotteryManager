			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						<div class="col-xs-12">
						<!-- 检索  -->
						<form action="userwithdraw/temporaryWithdrawList.do" method="post" name="Form" id="Form">
							<table style="margin-top:5px;border-collapse:separate; border-spacing:10px;" >
								<tr style="margin:2px ">
									...
									<c:if test="${QX.toExcel == 1 }">
										...
										<span class="input-icon" style="width:43px;"> </span>
											<span>
												<a class="btn btn-light btn-xs" onclick="toExcel();" title="导出到EXCEL"  style="border-radius:5px;color:blue !important; width:100px">导出到EXCEL </a>
											</span>
										</td>
									</c:if>
								</tr>
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
														...
														<c:choose>
															...
															<c:when test="${var.status == 2 }">	<a class="btn btn-xs btn-default" title="详情" style="border-radius: 5px;" onclick="edit('${var.id}');">详情</a></c:when>
														</c:choose>													
													</c:if>
												</div>
											</td>
	...
	<script type="text/javascript">
		...
		$(top.hangge());//关闭加载状态
			...
			function withdrawOperation(status,orderSn){
				...
				if(status==1 ){str+="确认该订单已提现吗?"}else{str+="确定要拒绝该订单吗?"}
...
// 						$.ajax({
// 							type: "POST",
<%-- 							url: '<%=basePath%>userwithdraw/withdrawOperation.do?', --%>
// 					    	data: {id:orderSn,status:status},
// 							dataType:'json',
// 							cache: false,
...
// 							},'json');
		...
		function edit(Id){	//修改
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>userwithdraw/goWithdrawDetail.do?id='+Id;
			 diag.Width =1200;
			 diag.Height = 450;
