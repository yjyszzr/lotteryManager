			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="phonechannel/list.do" method="post" name="Form" id="Form">
									...
									<td>
										<div class="nav-search">
											...
											<span class="input-icon">
												<input type="text" placeholder="渠道编号" class="nav-search-input" id="channel" autocomplete="off" name="channel" value="${pd.channel }"  />
											</span>
										</div>
									</td>
									<td>
...
<!-- 									<th class="center">id</th> -->
									...
									<th class="center">app编码</th>
									<th class="center">资讯类型</th>
...
<%-- 											<td class='center'>${var.id}</td> --%>
											...
											<td class='center'> 
												<c:forEach items="${var.articleclassifyList}" var="articleclassifys" varStatus="articleclassify">
														<div  class="a"> ${ articleclassifys.classify_name}</div>
												</c:forEach>
											</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" onclick="edit('${var.id}');"  style="border-radius: 5px;" >编辑</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.id}');"  style="border-radius: 5px;" >删除</a>
													</c:if>
												</div>
											</td>
										</tr>
...
// 		}
	...
	$("#Form").submit();
		...
		function del(Id){
			...
			bootbox.confirm(str+="确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>phonechannel/delete.do?id="+Id+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>phonechannel/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>phonechannel/excel.do';
		}
	</script>
