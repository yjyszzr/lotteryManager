...
public class ${objectName}Controller extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		pd = this.getPageData();
		pd.put("${primaryKey}_id", this.get32UUID());	//主键
<#list fieldList as var>
	...
	@ResponseBody
	public Object delete(@RequestParam String ${primaryKey}_id) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除${objectName}");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return null;} 					//校验权限
		Map<String,String> map = new HashMap<String,String>();
		PageData pd = new PageData();
		pd.put("${primaryKey}_id", ${primaryKey}_id);
		String errInfo = "success";
		if(${objectNameLower}Service.listByParentId(${primaryKey}_id).size() > 0){		//判断是否有子级，是：不允许删除
			errInfo = "false";
		}else{
	...
	public ModelAndView list(Page page) throws Exception{
		...
		String ${primaryKey}_id = null == pd.get("${primaryKey}_id")?"":pd.get("${primaryKey}_id").toString();
		if(null != pd.get("id") && !"".equals(pd.get("id").toString())){
			${primaryKey}_id = pd.get("id").toString();
		}
		pd.put("${primaryKey}_id", ${primaryKey}_id);					//上级ID
		page.setPd(pd);
		...
		mv.addObject("${primaryKey}_id", ${primaryKey}_id);			//上级ID
	...
	public ModelAndView listTree(Model model,String ${primaryKey}_id)throws Exception{
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try{
			...
			json = json.replaceAll("${primaryKey}_id", "id").replaceAll("PARENT_ID", "pId").replaceAll("NAME", "name").replaceAll("sub${objectName}", "nodes").replaceAll("has${objectName}", "checked").replaceAll("treeurl", "url");
			model.addAttribute("zTreeNodes", json);
			mv.addObject("${primaryKey}_id",${primaryKey}_id);
			mv.addObject("pd", pd);	
	...
	public ModelAndView goAdd()throws Exception{
		...
		mv.addObject("${primaryKey}_id", ${primaryKey}_id);			//传入ID，作为子级ID用
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		String ${primaryKey}_id = pd.getString("${primaryKey}_id");
		pd = ${objectNameLower}Service.findById(pd);							//根据ID读取		
		mv.addObject("pd", pd);													//放入视图容器
		pd.put("${primaryKey}_id",pd.get("PARENT_ID").toString());			//用作上级信息
		mv.addObject("pds",${objectNameLower}Service.findById(pd));				//传入上级所有信息
		mv.addObject("${primaryKey}_id", pd.get("PARENT_ID").toString());	//传入上级ID，作为子ID用
		pd.put("${primaryKey}_id",${primaryKey}_id);					//复原本ID
		pd = ${objectNameLower}Service.findById(pd);							//根据ID读取
		mv.setViewName("${packageName}/${objectNameLower}/${objectNameLower}_edit");
