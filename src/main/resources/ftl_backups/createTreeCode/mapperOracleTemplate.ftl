...
<mapper namespace="${objectName}Mapper">
	...
	<resultMap type="${objectName}" id="${objectNameLower}ResultMap">
		<id column="${primaryKey}_id" property="${primaryKey}_id"/>
	...
	<delete id="delete" parameterType="pd">
		...
		<include refid="tableName"></include>
		where 
			"${primaryKey}_id" = ${r"#{"}${primaryKey}_id${r"}"}
	</delete>
	...
	<select id="datalistPage" parameterType="page" resultType="pd">
		...
		<include refid="tableName"></include>
		where 1=1
		<if test="pd.${primaryKey}_id!= null and pd.${primaryKey}_id != ''"><!-- 检索 -->
		and "PARENT_ID" = ${r"#{"}pd.${primaryKey}_id${r"}"}
		</if>
