			...
			<div class="page-content">
				<div class="row">
					...
					<form action="${objectNameLower}/${r"${msg }"}.do" name="Form" id="Form" method="post">
						<input type="hidden" name="${primaryKey}_id" id="${primaryKey}_id" value="${r"${pd."}${primaryKey}_id${r"}"}"/>
...
<c:if test="${r"${'edit' == msg }"}">
	<div>
		<iframe name="treeFrame" id="treeFrame" frameborder="0" src="<%=basePath%>/${objectNameLower}mx/list.do?${primaryKey}_id=${r"${pd."}${primaryKey}_id${r"}"}" style="margin:0 auto;width:805px;height:368px;;"></iframe>
	</div>
</c:if>
