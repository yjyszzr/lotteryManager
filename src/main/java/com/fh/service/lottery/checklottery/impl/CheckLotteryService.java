...
public class CheckLotteryService implements CheckLotteryManager {
	...
	public PageData getTicketScheme(PageData param) throws Exception {
		...
		PageData orderInfoByOrderSn = (PageData) dao.findForObject("CheckLotteryMapper.getOrderInfoByOrderSn",
				param.get("orderSn").toString());
		...
		if (orderStatus < 10 && orderStatus > 2) {
			...
			Integer lotteryPlayClassifyId = Integer
					.parseInt(orderInfoByOrderSn.get("lottery_play_classify_id").toString());
			if (8 == lotteryPlayClassifyId && 1 == lotteryClassifyId) {
				List<PageData> orderDetails = (List<PageData>) dao.findForList("CheckLotteryMapper.selectByOrderId",
						orderInfoByOrderSn.get("order_id").toString());
				int i = 0;
				for (PageData detail : orderDetails) {
					...
					String ticketContent = playType + "(" + detail.get("matchTeam") + "["
							+ detail.get("ticketData").toString().split("@")[1] + "])";
	...
	public PageData getBetInfoByOrderSn(String orderSn) throws Exception {
		...
		if (allPlays != null) {
			for (PageData type : allPlays) {
				playTypeNameMap.put(Integer.parseInt(type.get("play_type").toString()),
						getCathecticData(Integer.parseInt(type.get("play_type").toString())));
				// play_name没值
			}
		}
	...
	private void betNum2(PageData str, int num, List<PageData> list, List<PageData> betList,
			Map<Integer, String> playTypeNameMap) {
		...
		while (link.size() > 0) {
			...
			if (Integer.valueOf(playType).equals(MatchPlayTypeEnum.PLAY_TYPE_HHAD.getcode())) {
				playName = remove.get("fixedodds") != null && !"".equals(remove.get("fixedodds"))
						? "[" + remove.get("fixedodds") + "]" + playName
						: playName;
			}
			...
			for (PageData betCell : betCells) {
				...
				String cellOdds = betCell.get("cellOdds") != null && !"".equals(betCell.get("cellOdds"))
						? betCell.get("cellOdds").toString()
						: "-";
				String betContent = str.get("betContent") + changci + "(" + playName + "_" + betCell.get("cellName")
						+ " " + cellOdds + ")X";
				if (num == 1) {
	...
	public List<PageData> getManualOrderList(PageData param) throws Exception {
		...
		List<PageData> orderList = (List<PageData>) dao.findForList("CheckLotteryMapper.getOrderInfoByOrderSn",
				param.get("order_sn").toString());
		for (PageData order : orderList) {
			...
			PageData lotteryClassify = (PageData) dao.findForObject("CheckLotteryMapper.lotteryClassify",
					lotteryClassifyId);
			...
			orderDetailDTO.put("ticketAmount",
					order.get("ticket_amount") != null ? order.get("ticket_amount").toString() : "");
			orderDetailDTO.put("surplus", order.get("surplus") != null ? order.get("surplus").toString() : "");
			orderDetailDTO.put("userSurplusLimit",
					order.get("user_surplus_limit") != null ? order.get("user_surplus_limit").toString() : "");
			orderDetailDTO.put("userSurplus",
					order.get("user_surplus") != null ? order.get("user_surplus").toString() : "");
			orderDetailDTO.put("thirdPartyPaid",
					order.get("third_party_paid") != null ? order.get("third_party_paid").toString() : "");
			...
			orderDetailDTO.put("winningMoney", order.get("winning_money"));
			orderDetailDTO.put("checkWinningStatus", order.get("check_winning_status"));
			orderDetailDTO.put("createTime", WeekDateUtil.getCurrentTimeString(
					Long.parseLong(order.get("add_time").toString()), WeekDateUtil.datetimeFormat));
			long acceptTime = Long.parseLong(order.get("accept_time").toString());
			if (acceptTime > 0) {
				orderDetailDTO.put("acceptTime",
						WeekDateUtil.getCurrentTimeString(acceptTime, WeekDateUtil.datetimeFormat));
			} else {
				orderDetailDTO.put("acceptTime", "--");
			}
			...
			if (ticketTime > 0) {
				orderDetailDTO.put("ticketTime",
						WeekDateUtil.getCurrentTimeString(ticketTime, WeekDateUtil.datetimeFormat));
			} else {
				orderDetailDTO.put("ticketTime", "--");
			}
			List<PageData> playTypes = (List<PageData>) dao.findForList("CheckLotteryMapper.getPlayTypes",
					lotteryClassifyId);
			Map<Integer, String> playTypeNameMap = new HashMap<Integer, String>();
			if (playTypes != null) {
				for (PageData type : playTypes) {
					playTypeNameMap.put(Integer.valueOf(type.get("playType").toString()),
							type.get("playName").toString());
				}
			}
			...
			if (CollectionUtils.isNotEmpty(orderDetails)) {
				...
				for (PageData orderDetail : orderDetails) {
					...
					if (isWorlCup) {
						...
						matchInfo.put("cathecticResults", this.getCathecticResults(fixedodds,
								String.valueOf(orderDetail.get("ticketData")), matchResult, playTypeNameMap));
					}
	 ...
	 * @return
	 */
	private List<PageData> getCathecticResults(String fixedodds, String ticketData, String matchResult,
		...
		if (CollectionUtils.isNotEmpty(ticketDatas)) {
			for (String temp : ticketDatas) {
				...
				for (int i = 0; i < betCellArr.length; i++) {
					...
					cathectic.put("cathectic",
							cathecticStr + "[" + String.format("%.2f", Double.valueOf(betCellOdds)) + "]");
					if (null != matchResults) {
						for (String matchStr : matchResults) {
							String rstPlayType = matchStr.substring(0, matchStr.indexOf("|"));
							String rstPlayCode = matchStr.substring(matchStr.indexOf("|") + 1,
	...
	private String getCathecticData(String playTypeStr, String cathecticStr) {
		...
		if (MatchPlayTypeEnum.PLAY_TYPE_HHAD.getcode() == playType
				|| MatchPlayTypeEnum.PLAY_TYPE_HAD.getcode() == playType) {
	...
	public void checkPrize(PageData pd) throws Exception {
		dao.update("CheckLotteryMapper.checkPrize", pd);
	}
}
