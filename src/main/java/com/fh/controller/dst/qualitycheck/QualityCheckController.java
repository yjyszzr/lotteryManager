package com.fh.controller.dst.qualitycheck;
...
public class QualityCheckController extends BaseController {
	...
	String menuUrl = "qualitycheck/list.do"; //菜单地址(权限用)
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增QualityCheck");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "add")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
//		pd.put("quality_id", this.get32UUID());	//主键
//		pd.put("purchase_code", "");	//业务数据编码
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除QualityCheck");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		qualitycheckService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改QualityCheck");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		qualitycheckService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表QualityCheck");
		...
		page.setPd(pd);
		List<PageData>	varList = qualitycheckService.list(page);	//列出QualityCheck列表
		mv.setViewName("erp/qualitycheck/qualitycheck_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("erp/qualitycheck/qualitycheck_edit");
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = qualitycheckService.findById(pd);	//根据ID读取
		mv.setViewName("erp/qualitycheck/qualitycheck_edit");
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除QualityCheck");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			qualitycheckService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出QualityCheck到excel");
		...
		List<PageData> varOList = qualitycheckService.listAll(pd);
