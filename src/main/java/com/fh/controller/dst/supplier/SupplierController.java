package com.fh.controller.dst.supplier;
...
import com.fh.service.dst.supplier.SupplierManager;
...
//		pd.put("supplier_id", this.get32UUID());	//主键
	...
	public ModelAndView list(Page page) throws Exception{
		...
		String supplier_name = pd.getString("supplier_name");				//关键词检索条件
		if(null != supplier_name && !"".equals(supplier_name)){
			pd.put("supplier_name", supplier_name.trim());
		}
		page.setPd(pd);
		List<PageData>	varList = supplierService.list(page);	//列出Supplier列表
		mv.setViewName("dst/supplier/supplier_list");
	...
	public ModelAndView listForSelect(Page page) throws Exception{
		...
		page.setPd(pd);
		List<PageData>	varList = supplierService.list(page);	//列出Supplier列表
		mv.setViewName("dst/supplier/supplier_list_select");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		mv.addObject("QX",Jurisdiction.getHC());	//按钮权限
		return mv;
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		String code=getSerialConfigUtilService.getSerialCode(FormType.SUPPLIER_CODE);
		pd.put("supplier_sn", code);
		mv.setViewName("dst/supplier/supplier_edit");
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var1", varOList.get(i).get("supplier_id").toString());	//1
			vpd.put("var2", varOList.get(i).get("supplier_type").toString());	//2
			vpd.put("var3", varOList.get(i).getString("supplier_sn"));	    //3
			vpd.put("var4", varOList.get(i).getString("supplier_name"));	    //4
			vpd.put("var5", varOList.get(i).getString("deposit"));	    //5
			vpd.put("var6", varOList.get(i).getString("taxes"));	    //6
			vpd.put("var7", varOList.get(i).getString("supplier_remark"));	    //7
			vpd.put("var8", varOList.get(i).get("is_show").toString());	//8
			vpd.put("var9", varOList.get(i).get("is_deleted").toString());	//9
			vpd.put("var10", varOList.get(i).get("admin_id").toString());	//10
			varList.add(vpd);
		}
	...
	public Map<String, String> refreshCache() throws Exception {
		...
		pd = this.getPageData();
		try {
			RedisUtil.setSupplierInfo(supplierService.listAll(pd));
		} catch (Exception e) {
