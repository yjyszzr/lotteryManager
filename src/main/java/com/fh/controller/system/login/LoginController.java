...
public class LoginController extends BaseController {
	...
	public Object login() throws Exception {
		...
		if (null != KEYDATA && KEYDATA.length == 3) {
			...
			// session.setAttribute(Const.SESSION_SECURITY_CODE, "112233");
			String sessionCode = (String) session.getAttribute(Const.SESSION_SECURITY_CODE); // 获取session中的验证码

			String code = KEYDATA[2] + "," + KEYDATA[0].trim();
	...
	public ModelAndView login_index(@PathVariable("changeMenu") String changeMenu) {
		...
		try {
			...
			if (user != null) {
				...
				// String[] stores = new String[pageDatas.size()];
				// if(CollectionUtils.isNotEmpty(pageDatas)) {
				// for (int i=0;i<pageDatas.size();i++) {
				// stores[i] = pageDatas.get(i).getString("store_sn");
				// }
				// }
	...
	public void setAttributeToAllDEPARTMENT_ID(Session session, String USERNAME) throws Exception {
		...
		// }
		session.setAttribute(Const.DEPARTMENT_IDS, "无权"); // 把用户的组织机构权限集合放到session里面
		session.setAttribute(Const.DEPARTMENT_ID, "无权"); // 把用户的最高组织机构权限放到session里面
	}
	...
	@RequestMapping(value = "login_smsCode")
	@ResponseBody
	public Object smsCode() throws Exception {
		Map<String, String> map = new HashMap<String, String>();
		PageData pd = new PageData();
		pd = this.getPageData();
