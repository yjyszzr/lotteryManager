...
/**
 * 说明：超级白名单 创建人：FH Q313596790 创建时间：2018-12-26
 */
@Controller
...
//		mv.addObject("pd", pd);
	...
	public ModelAndView deduction() throws Exception {
		...
		if (!Jurisdiction.buttonJurisdiction(menuUrl, "deduction")) {
			return null;
		} // 校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
	...
	public ModelAndView refound() throws Exception {
		...
		if (!Jurisdiction.buttonJurisdiction(menuUrl, "refound")) {
			return null;
		} // 校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
	...
	public ModelAndView recharge() throws Exception {
		...
		if (!Jurisdiction.buttonJurisdiction(menuUrl, "recharge")) {
			return null;
		} // 校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
