...
public class AppDocController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		pd = this.getPageData();
//		pd.put("app_id", this.get32UUID());	//主键
		pd.put("create_time", DateUtilNew.getCurrentTimeLong());	//备注4
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除AppDoc");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			appdocService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出AppDoc到excel");
		...
		List<PageData> varOList = appdocService.listAll(pd);
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var4", varOList.get(i).get("create_time").toString());	//4
			varList.add(vpd);
		}
